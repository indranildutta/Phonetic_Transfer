...
with(df_lobanov_gender_time, plotVowels(f1, f2, vowel, group = interaction(task,context),
                               plot.tokens = F, plot.means = TRUE , pch.tokens = vowel, pch.means = interaction(task,context),
                               var.col.by = interaction(task,context), var.sty.by = gender, 
                               cex.tokens = 0.8, cex.means = 1, alpha.means = 0.5,alpha.tokens = 0.3,
                               ellipse.line = F,  ellipse.fill = F,
                               poly.line = F, poly.order = c("\\ae","\\vt"),
...
# raw FFs
with(df_time, plotVowels(f1, f2, vowel, group = interaction(gender, task), 
                               plot.tokens = F, plot.means = TRUE , pch.tokens = vowel, pch.means = vowel,
                               var.col.by = interaction(gender, task), var.sty.by = task, 
                               cex.tokens = 0.8, cex.means = 1, alpha.means = 0.5,alpha.tokens = 0.3,
                               ellipse.line = F,  ellipse.fill = F,
                               poly.line = F, poly.order = c("\\ae","\\vt"),
...
beta.f1<- beta.f1 %>%
  map(~ . * 0.95) %>% # map each element of the vector to the result of applying the function f to it (creates a list)
  unlist()  # convert back to a named vector

beta.f1

power_f1_minus5 <- mixedpower(model = phon_acco.model_f1, data = temp,
                          fixed_effects = c("vowel","context","time","task"),
...
powerplot_f1_minus5 <- multiplotPower(power_f1_minus5, filename = "powerplot_f1_minus5.png")
...
# setting beta-values to 85% of original
beta.f2 <- beta.f2 %>%
  map(~ . * 1.15) %>% # map each element of the vector to the result of applying the function f to it (creates a list)
  unlist()  # convert back to a named vector

beta.f2

power_f2_plus15 <- mixedpower(model = phon_acco.model_f2, data = temp,
                          fixed_effects = c("vowel","context","time","task"),
                          simvar = "subject_num", steps = c(5,7,10,12,15), n_sim = 500,
                          SESOI = beta.f2, databased = F,
                          critical_value = 2)

powerplot_f2_plus15 <- multiplotPower(power_f2_plus15, filename = "powerplot_f2_plus15.png")
