...
df_ben_AE <- df_ben %>%
  filter(grepl("A|E", vowel), time %in% (8:12), f1>500) %>%
  droplevels() %>%
...
df_clean <- df_ben_long %>%
  select(speaker, gender, vowel, time, formant, ff_log_normalized, ff_exp_normalized) 
...
df_ben_normalized <- bind_cols(f1, f2$ff_log_normalized, f2$ff_exp_normalized) %>%
  mutate(formant=NULL) %>%
  rename(f1_log=ff_log_normalized, f1_exp=ff_exp_normalized, f2_log=...8, f2_exp=...9)


rm(df_clean, f1, f2)


variance <- var(df_ben$f1)
variancef2 <- var(df_ben$f2)
...
# Bengali vowels
with(df_ben_normalized,plotVowels(f1_log, f2_log, vowel,
                        ...
                        ellipse.line = F, ellipse.fill = T, fill.opacity = 0.5,
...
# English vowels
with(df_normalized, plotVowels(f1_log, f2_log, vowel, group = context, 
                               plot.tokens = F, plot.means = TRUE , pch.tokens = unicodevowel, pch.means = unicodevowel,
