...
\begin{abstract}
This study investigates the phonetic outcome of mixed-language production in a group of highly proficient bilingual speakers of Bengali and English, to better understand the nature of cross-language phonetic interaction. Existing research has largely focused on a small set of phonologically related languages and used voice onset time (VOT) to measure transfer. Results show that transfer is mediated by both language-specific features and linguistic experience of participants, pointing to an urgent need for diversifying the range of languages and populations studied. We compare productions from a yet-unstudied population in two conditions—unilingual L2 (English) and mixed (English and Bengali). The spectral properties of two L2 vowel categories— \nt{\ae}(common to L1 and L2) and \nt{2} (absent in L1) are examined as sites for transfer. To understand how this unfolds dynamically, we measure F1 and F2 at five points over the course of articulating the vowel. We find a dynamic change in both vowels during mixed production, with \nt{2} shifting towards a related L1 category. We interpret this as a temporary increase in cross-language phonetic influence during mixed-language use, leading to a shift towards L1 norms. asymmetrical shifts in the two categories, and no difference in transfer patterns between the two paradigms. 
\end{abstract}
