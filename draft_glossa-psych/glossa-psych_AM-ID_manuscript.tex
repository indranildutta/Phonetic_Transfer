...
\begin{abstract}
This study investigates the phonetic outcome of mixed-language production in a group of highly proficient bilingual speakers of Bengali and English, to better understand the nature of cross-language phonetic interaction. Existing research has largely focused on a small set of phonologically related languages and used voice onset time (VOT) to measure phonetic transfer. Results show that this interaction is mediated by both language-specific features and linguistic experience of participants, necessitating diversification in the range of languages and populations studied. We compare productions from a yet-unstudied population in two conditions-- unilingual L2 (English) and mixed (Bengali and English). The spectral properties of two L2 vowel categories— \nt{\ae}(common to L1 and L2) and \nt{2} (absent in L1) are examined as sites for transfer. To understand how this unfolds dynamically, we measure F1 and F2 at five points over the course of articulating the vowel. We find dynamic and  asymmetrical changes in both vowels during mixed production, with \nt{2} shifting towards a related L1 category. We interpret this as a temporary increase in cross-language phonetic influence during mixed-language use, leading to a shift towards L1 norms. Mixed language is elicited in two switching paradigms: cued picture-naming and code-switching, to assess if differences between the paradigms independently influence the outcome of phonetic interaction. Results reveal identical transfer patterns across the two paradigms. Findings are discussed in light of recent proposals about asymmetries in short-term phonetic interaction and the role of connected speech in introducing discursive factors to transfer studies. 
\end{abstract}
...
populations, language pairs,  and phonetic features needs to be considered in order to make meaningful generalizations. Widespread multilingualism in the Indian subcontinent suggests that phonetic
%line--rephrase
...
\subsection{Duration of effects} \label{duration}
...
Gloss and translation for Bengali sentence:
	...
	o-\textipa{\:t}a \quad \textipa{\ae}k-\textipa{\:t}a \quad kalo [target word]\\
